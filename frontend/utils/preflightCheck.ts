...
export async function runPreflightCheck(
  ...
  for (let i = 0; i < framesToSample; i++) {
    try {
      ...
      // Simple error handling: just log and continue to next frame
      // This ensures we don't break the entire loop and get empty arrays
...
async function analyzeImage(photo: { base64?: string | null; exif?: any }): Promise<{ brightness: number; sharpness: number } | null> {
  if (Platform.OS === 'web') {
    ...
    // Try web approach first if Canvas-like APIs are available (for better stability)
    // Otherwise fall back to native implementation
    if (photo.base64 && typeof document !== 'undefined' && typeof Image !== 'undefined') {
      try {
        return await analyzeImageWeb(photo.base64);
      } catch (e) {
        console.warn("Web analysis failed, falling back to native:", e);
      }
    }
    // Native implementation (iOS/Android) — improved stability
...
// Native implementation - improved stability with better error handling and EXIF fallback
async function analyzeImageNative(photo: { base64?: string | null; exif?: any }): Promise<{ brightness: number; sharpness: number } | null> {
  try {
    // Validate input
    if (!photo.base64) {
      console.warn("No base64 data available for analysis");
      return null;
    }
    ...
    // Prepare bytes for analysis with better error handling
    ...
    try {
      ...
      if (bytes.length === 0) {
        console.warn("Empty byte array from base64 decode");
        return null;
      }
    } catch (e) {
      console.warn("Failed to decode base64:", e);
      // If we have EXIF brightness, we can still return that
      if (brightnessFromExif != null) {
        return { brightness: brightnessFromExif, sharpness: 0.05 }; // Default sharpness if we can't calculate
      }
      return null;
    }
    
    // Estimate sharpness from JPEG bytes variance (improved sampling)
    let sharpness = 0;
    if (bytes && bytes.length > 0) {
      // Use larger sample size for better accuracy
      const sampleSize = Math.min(20000, bytes.length);
      const step = Math.max(1, Math.floor(bytes.length / sampleSize));
      let sum = 0;
      let variance = 0;
      const samples: number[] = [];
      
      // Sample more evenly across the image
      ...
      if (samples.length === 0) {
        sharpness = 0.05; // Default fallback
      } else {
    ...
    // Determine brightness: EXIF-derived preferred, else improved heuristic from bytes
    let brightness: number;
    if (brightnessFromExif != null) {
      brightness = brightnessFromExif;
    } else if (bytes && bytes.length > 0) {
      // Improved brightness calculation using larger sample
      const sampleSize = Math.min(20000, bytes.length);
      const step = Math.max(1, Math.floor(bytes.length / sampleSize));
      let sum = 0;
      let count = 0;
      ...
      // Improved scaling factor based on web implementation comparison
      brightness = mean * 0.85; // Slightly adjusted to better match RGB brightness range
    } else {
      return null;
    }
